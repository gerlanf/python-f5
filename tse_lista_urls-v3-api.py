...
def create_statistics_profile_api(base_url, token, profile_name, fields):
    url_profile = f'{base_url}/mgmt/tm/ltm/profile/statistics/{profile_name}'
    headers = {'X-F5-Auth-Token': token, 'Content-Type': 'application/json'}
    profile_content = {
        "name": profile_name,
        "partition": "Common",
    ...
    response = requests.get(url_profile, headers=headers, verify=False)
    if response.status_code == 200:
        overwrite = input(f'O profile {profile_name} já existe. Deseja sobrescrever? (s/n): ').strip().lower()
        if overwrite != 's':
            return
    try:
        response = requests.put(url_profile, headers=headers, json=profile_content, verify=False)
        response.raise_for_status()
        print(f'Profile statistics criado/sobrescrito via API: {profile_name}')
    except Exception as e:
...
def create_irule_api(base_url, token, irule_name, irule_content):
    url_irule = f'{base_url}/mgmt/tm/ltm/rule/{irule_name}'
    headers = {'X-F5-Auth-Token': token, 'Content-Type': 'application/json'}
    irule_data = {
        "name": irule_name,
        "apiAnonymous": irule_content
    }
    response = requests.get(url_irule, headers=headers, verify=False)
    if response.status_code == 200:
        overwrite = input(f'A iRule {irule_name} já existe. Deseja sobrescrever? (s/n): ').strip().lower()
        if overwrite != 's':
            return
    try:
        response = requests.put(url_irule, headers=headers, json=irule_data, verify=False)
        response.raise_for_status()
        print(f'iRule criada/sobrescrita via API: {irule_name}')
    except Exception as e:
